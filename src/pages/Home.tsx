    ...
    <>
      ...
      <main className="bg-[radial-gradient(circle_at_top_left,#ffffff_0%,#f8fbff_44%,#edf6ff_100%)] text-slate-950">
          ...
          <Container>
            <div className="grid items-start gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-12">
              <div>
                ...
                <h1 className="mt-4 max-w-[560px] font-display text-[2.65rem] font-black leading-[0.98] tracking-[-0.055em] text-slate-950 sm:text-6xl lg:text-[4rem]">
                  One workflow, end-to-end — from calendar invite to manager update.
                </h1>
                <p className="mt-6 max-w-[560px] text-[1.05rem] font-medium leading-8 text-slate-700">
                ...
                <div className="relative mt-8 h-[545px] max-w-[650px] sm:mt-10">
                  ...
                  {WORKFLOW.map((step) => {
                    ...
                    return (
                      <div
                        key={step.label}
                        className={`absolute flex items-center gap-4 ${step.align === 'left' ? '-translate-x-[98%] -translate-y-1/2 text-right' : ''} ${step.align === 'right' ? '-translate-y-1/2 text-left' : ''} ${step.align === 'center' ? '-translate-x-1/2 -translate-y-[18%] flex-col text-center' : ''} ${step.align === 'bottom' ? '-translate-x-1/2 -translate-y-[12%] flex-col text-center' : ''}`}
                      ...
                      >
                        ...
                        <div className="max-w-[148px]">
                          <p className={`text-xl font-black leading-none ${step.color}`}>{step.number}</p>
                          <h3 className="mt-1 text-[0.92rem] font-black leading-tight text-slate-950">{step.label}</h3>
                          <p className="mt-1 text-[0.78rem] font-semibold leading-5 text-slate-700">{step.desc}</p>
                        </div>
                      </div>
                    )
                  })}
              ...
              <div className="rounded-[2rem] border border-slate-200 bg-white/95 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.08)] backdrop-blur sm:p-8 lg:mt-8">
                <div className="mb-8 flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-black uppercase tracking-[0.18em] text-primary">What you get</p>
                    <h2 className="mt-3 max-w-[420px] font-display text-[2rem] font-black leading-[1.05] tracking-[-0.035em] text-slate-950">
                      Eight outputs from one meeting.
                    </h2>
                  </div>
                  <span className="grid h-20 w-20 shrink-0 place-items-center rounded-full bg-blue-50 text-center text-sm font-black leading-tight text-primary">
                    Every<br />time
                  </span>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {OUTPUTS.map((output) => {
                    ...
                    return (
                      <div key={output.title} className="flex min-h-[120px] items-start gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg">
                        <div className={`grid h-14 w-14 shrink-0 place-items-center rounded-xl ${output.bg} ${output.color}`}>
                          <Icon className="h-7 w-7" />
                        </div>
                        <div>
                          <h3 className="text-lg font-black leading-tight text-slate-950">{output.title}</h3>
                          <p className="mt-2 text-base font-medium leading-6 text-slate-700">{output.desc}</p>
                        </div>
                      </div>
                    )
                  })}
                ...
                <div className="mt-6 flex items-center justify-center gap-3 rounded-2xl border border-blue-200 bg-blue-50 px-6 py-5 text-center">
                  <ShieldCheck className="h-6 w-6 shrink-0 text-primary" />
                  <p className="text-base font-black text-primary">All connected. All automated. All in one place.</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 grid overflow-hidden rounded-[2rem] border border-slate-200 bg-white/95 shadow-[0_18px_50px_rgba(15,23,42,0.06)] sm:grid-cols-2 lg:grid-cols-4">
              {BENEFITS.map((benefit) => {
                ...
                return (
                  <div key={benefit.title} className="flex items-start gap-4 border-slate-200 p-7 lg:border-r last:border-r-0">
                    <Icon className="h-9 w-9 shrink-0 text-primary" />
                    <div>
                      <h3 className="text-lg font-black leading-tight text-slate-950">{benefit.title}</h3>
        ...
        <section className="px-6 pb-12 pt-4 lg:pb-16">
          <Container>
            <div className="grid items-stretch gap-8 lg:grid-cols-[0.95fr_1.55fr]">
              <div>
              ...
              <div className="grid rounded-[2rem] border border-slate-200 bg-white/95 p-7 shadow-[0_18px_50px_rgba(15,23,42,0.06)] sm:grid-cols-[1fr_0.7fr_0.9fr] sm:p-8">
                <div className="flex gap-6 pr-6">
                  <div className="grid h-20 w-20 shrink-0 place-items-center rounded-full bg-blue-50 text-primary">
                    <Rocket className="h-10 w-10" />
                  </div>
                  <div>
                    <h3 className="font-display text-3xl font-black leading-tight tracking-[-0.035em] text-slate-950">
                ...
                <div className="mt-8 border-slate-200 sm:mt-0 sm:border-l sm:px-8">
                ...
                <div className="mt-8 border-slate-200 sm:mt-0 sm:border-l sm:pl-8">
...
function FitCard({ title, items, positive = false }: { title: string; items: string[]; positive?: boolean }) {
  return (
    <div className={`rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_14px_36px_rgba(15,23,42,0.06)] ${positive ? 'bg-gradient-to-br from-white to-green-50/60' : 'bg-gradient-to-br from-white to-red-50/40'}`}>
...
function SocialLink({ icon: Icon, label, href }: { icon: typeof Linkedin; label: string; href: string }) {
  return (
